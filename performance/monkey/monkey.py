...
def capture_screen(device_id, log_file_name, log_file_name_with_location, monkey_duration):
    ...
    logging.info(log_file_name_with_location)
    cmd_screenshot = "%s -s %s shell screencap -p | sed s/\r$// > %s.png" % (adb, device_id, log_file_name_with_location)
#    cmd_capture = "%s -s %s shell screencap -p /sdcard/%s.png" % (adb, device_id, log_file_name)
    logging.info(cmd_screenshot)
    logging.info(log_file_name)
    status, output = commands.getstatusoutput(cmd_screenshot)
...
#        logging.info(cmd_pull_screenshot)
